...
namespace  Huanr.NativeSoilWebApp.Tools.AlipayWapDirect
{
    ...
    public class Config
    {
        ...
        private string _service = "alipay.wap.create.direct.pay.by.user";
        ...
        private void Init()
        {
            try
            {
                ...
                object DocCacheObj = zeroService.Cache.Get<System.Xml.XmlDocument>(CacheKey);
                string XmlFilePath = "";
                ...
                System.Xml.XmlNode alipay_wap_direct_node = Doc.SelectSingleNode(@"payment_config/alipay_wap_direct");
                if (alipay_wap_direct_node == null)
                {
                    throw new ArgumentException("配置文件缺少配置节：alipay_direct");
                }

                #region --读取逐个配置节的value--
                System.Xml.XmlNode node = alipay_wap_direct_node.SelectSingleNode(@"partner");
                ...
                node = alipay_wap_direct_node.SelectSingleNode(@"seller_id");
                ...
                node = alipay_wap_direct_node.SelectSingleNode(@"return_url");
                ...
                node = alipay_wap_direct_node.SelectSingleNode(@"sign_type");
                ...
                node = alipay_wap_direct_node.SelectSingleNode(@"log_path");
                ...
                node = alipay_wap_direct_node.SelectSingleNode(@"input_charset");
                ...
                node = alipay_wap_direct_node.SelectSingleNode(@"payment_type");
                ...
                node = alipay_wap_direct_node.SelectSingleNode(@"service");
                ...
                if (IsReadyByFile)
                {
                    //NativeSoil.Core.Common.CacheHelper.Insert(CacheKey, Doc, XmlFilePath);
                    zeroService.Cache.Set<System.Xml.XmlDocument>(CacheKey,Doc,DateTime.Now.AddMinutes(5));
                }
            }
            catch (Exception Ex)
            {
