...
namespace ZeroDbs.DataAccess.Common
{
    internal static class DbDataviewStructCache
    {
        class StructCache
        {
            ...
            public ZeroDbs.Interfaces.Common.DbDataTableInfo CacheData { get; set; }
        }
        static readonly int CacheMinutes = 60 * 24;
        static Dictionary<string, StructCache> CacheDic = new Dictionary<string, StructCache>();
        public static ZeroDbs.Interfaces.Common.DbDataTableInfo Get(string key)
        {
        ...
        public static void Set(string key, ZeroDbs.Interfaces.Common.DbDataTableInfo value)
        {
            if (string.IsNullOrEmpty(key))
            {
                throw new Exception("key不能为空");
