            ...
            if (ExecuteCount > 0)
            {
                if (!completedFlag)
                {
                    ...
                    if (isSuccess)
                    {
                        ...
                        FireEvent(new DbExecuteSqlEventArgs(db.Database.UseKey, new List<string>(), DbExecuteSqlType.TRANSACTION, "事务" + this.Identification + "(" + this.GroupId + ")已经提交"));
                    }
                    else
                    {
                        ...
                        {
                            FireEvent(new DbExecuteSqlEventArgs(db.Database.UseKey, new List<string>(), DbExecuteSqlType.TRANSACTION, "事务" + this.Identification + "(" + this.GroupId + ")已经回滚"));
                        }
                        else
                        {
                            FireEvent(new DbExecuteSqlEventArgs(db.Database.UseKey, new List<string>(), DbExecuteSqlType.TRANSACTION, "事务" + this.Identification + "(" + this.GroupId + ")已经回滚(" + executeExceptionMsg + ")"));
                        }
                    }
                }
                else
                {
                    FireEvent(new DbExecuteSqlEventArgs(db.Database.UseKey, new List<string>(), DbExecuteSqlType.TRANSACTION, "事务" + this.Identification + "(" + this.GroupId + ")已经执行过Complete()"));
                }
            }
            Dispose();
        }
